...
async function ensureTable() {
  ...
  `;
  // 대량생산으로 전환하면서 작품당 1점 한정 제약은 더 이상 유효하지 않다.
  // 예전에 만들어둔 work_id 유니크 인덱스가 남아있으면 같은 작품을 여러
  // 명이 주문할 수 없게 막아버리므로, 있으면 제거한다.
  await sql`DROP INDEX IF EXISTS reservations_work_id_unique`;
  ensured = true;
}
